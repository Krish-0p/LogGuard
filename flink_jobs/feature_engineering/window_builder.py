...
class HostWindowAggregator(WindowFunction):
    ...
    def apply(self, key: str, window, inputs):  # <-- REMOVED 'out' HERE
        events = list(inputs)
        ...
        template_counts = Counter(e.get("template_id") for e in events)
        ...
        response_times = [e.get("response_time_ms") for e in events if e.get("response_time_ms") is not None]
        ...
        for template_id, count in template_counts.items():
            ...
            template_vector[vocab_idx] += count / total_logs
        ...
        # <-- RETURN AS A LIST INSTEAD OF out.collect() -->
