...
def send_logs(count: int = 1000, anomaly_rate: float = 0.05):
    ...
    for i in range(count):
        ...
        # SPREAD THE LOGS EVENLY ACROSS 10 MINUTES
        current_log_time = start_time + timedelta(seconds=(i * (600 / count)))
        
        log_event = {
            "log_id": f"log_{i}_{int(time.time())}",
            "timestamp": int(current_log_time.timestamp() * 1000), # Simulated timestamp
    ...
    producer.flush()
    print(f"✅ Sent {count} logs spanning 10 simulated minutes!")
